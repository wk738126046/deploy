...
import importlib
import sys
from symbol.symbol_factory import get_symbol_split
...
if __name__ == '__main__':
    args = parse_args()
    det, out = get_symbol_split(args.network, args.data_shape,
        num_classes=args.num_classes, nms_thresh=args.nms_thresh,
        force_suppress=args.force_nms, nms_topk=args.nms_topk, tvm = True)
    ...
    if not os.path.exists('./model/deploy'):
        os.mkdir('./model/deploy')

    tmp = prefix.rsplit('/', 1)
    save_prefix = '/deploy/deploy_'.join(tmp)

    save_prefix_det = save_prefix + '-det'
    
    save_prefix_out = save_prefix + '-out'

    mx.model.save_checkpoint(save_prefix_det , args.epoch, det, arg_params, aux_params)
    
    out.save('{}-symbol.json'.format(save_prefix_out))

    print("Saved model: {}-{:04d}.params".format(save_prefix_det, args.epoch))
    
    print("Saved symbol: {}-symbol.json".format(save_prefix_det)) # group
    
    print("Saved symbol: {}-symbol.json".format(save_prefix_out)) # MultiBoxDetection output[ batch, ]
