...
def get_symbol_split(network, num_classes, from_layers, num_filters, sizes, ratios,
               strides, pads, normalizations=-1, steps=[], min_filter=128,
               nms_thresh=0.5, force_suppress=False, nms_topk=400, **kwargs):
    """Build network for testing SSD
    ...
    cls_prob_var = mx.sym.var('cls_prob')

    loc_preds_var = mx.sym.var('loc_preds')

    anchor_boxes_var = mx.sym.var('anchor_boxes')

    out = mx.symbol.contrib.MultiBoxDetection(*[cls_prob_var, loc_preds_var, anchor_boxes_var], \
        name="detection", nms_threshold=nms_thresh, force_suppress=force_suppress,
        variances=(0.1, 0.1, 0.2, 0.2), nms_topk=nms_topk)
    print('output: ',out)
    return (mx.sym.Group([cls_prob, loc_preds, anchor_boxes]), out)